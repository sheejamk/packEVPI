...
test_that("testing evpi and evppi for subset of parameters  but for mulitple
    ...
    tm <- packDAMipd::populate_transition_matrix(3, tmat, c("p_HH", "p_HS",
                                                            "p_HD",
                                                            "p_SS", "p_SD",
                                                            "p_DD"),
                                                 colnames(tmat))
    ...
    param_list <-
      packDAMipd::define_parameters(
        ...
        p_HS  = packDAMipd::get_parameter_def_distribution("p_HS", param_file,
    ...
    D <- packDAMipd::health_state("D", cost = "c_D ", utility = "u_D")
    health_states <- packDAMipd::combine_state(H, S, D)
    trt_strategy <- packDAMipd::strategy(tm, health_states, "New treatment")
    trt_markov <- packDAMipd::markov_model(current_strategy = trt_strategy,
                                           cycles = 85,
