...
#' @param names_params_needed names of needed parameters  from param matrix returned
...
#' @param param_file all parameters required to run the model,provided with name of
#' parameter, distribution and parameters that define the probability distribution
#' @param colnames_paramdistr col names where the parameter distribution is defined
...
#' give it back as it is. If the parameter is not found, it wil be taken same that
#' as that of in the model parameter.
get_parameter_list <- function(param_interest,value_param_interest,
  ...
  param_colno <- IPDFileCheck::get_columnno_fornames(param_distr_data, "parameter")
  if (length(param_colno) > 1) {
    stop("Parameter column should be only one")
  }

  param_colname = colnames(param_distr_data)[param_colno]
  distr_col_no = IPDFileCheck::get_columnno_fornames(param_distr_data, "distribution")
  ...
  for (j in 2:length(names_params_needed)) {
    ...
    if (names_params_needed[j] != param_interest) {
      ...
      if (names_params_needed[j] %in% names_params_model) {
        ...
        # is a numerical value, that has been either read from afile or directly
        # assigned. It might not be calculated. so those can be found from
        # the parameter file
        res = suppressWarnings(as.numeric(params_passed[names_params_needed[j]]))
        if (!is.na(res)) {
          # check if the parameter is defined using a distribution or actually fixed (for death rates)
          row_cor_param = param_distr_data[param_distr_data[param_colname] == names_params_needed[j],]
          if (nrow(row_cor_param) > 1) {
            stop("Error - Only one row should correspond to the given paramter in parameter file")
          }
          if (nrow(row_cor_param) == 0) {
            curr_parm_value = params_passed[names_params_needed[j]]
          }else{
            distr_col_val_param = row_cor_param[[distr_col_no]]
            # if the entry corresponding to distribution is null, na or empty keep this param fixed
            if (packDAMipd::check_null_na(distr_col_val_param) < 0 | distr_col_val_param == "") {
              curr_parm_value = packDAMipd::get_parameter_read(names_params_needed[j], param_file)
            }else{
              curr_parm_value = packDAMipd::get_parameter_def_distribution(names_params_needed[j], param_file,
                                                               colnames_paramdistr)
            }
          }
        }else{
...
#' @param param_file all parameters required to run the model,provided with name of
#' parameter, distribution and parameters that define the probability distribution
#' @param colnames_paramdistr column names where parameter distribution and values are defined
...
#' @return icer_nmb ICER and NMBs for the set of parameters and given strategies
#' @keywords internal
#' @details
...
find_nmb_setofparams <- function(param_interest,value_param_interest,
  ...
  for (i in 1:nrow(list_markov)) {
    ...
    if (length(names_params_needed) <= 1) {
      stop("Error - Check the columns of the parameter matrix provided by the markov model")
    }
    names_params_model = names(list_markov[i,]$list_param_values)
    if (length(names_params_model) < 1) {
      stop("Error - Check the parameters passed to the markov model")
    }
    check = names_params_needed[2:length(names_params_needed)] %in% names_params_model
    if (sum(check) != length(names_params_model))
      stop("Error - Parameters passed and returned to the model should be simmiar")
    params_passed = list_markov[i,]$list_param_values
    current_param_list <- get_parameter_list(param_interest,value_param_interest,
                                             names_params_needed,names_params_model,
                                             params_passed,param_file,colnames_paramdistr)

    this_markov  = packDAMipd::markov_model(this$strategy, this$cycles, this$initial_state,
                                this$discount, current_param_list,
                                this$method, this$half_cycle_correction)
    new_list_markov[[length(new_list_markov) + 1]] = this_markov
  }
  new_list_markov = packDAMipd::combine_markov(new_list_markov)
  icer_nmb = packDAMipd::calculate_icer_nmb(new_list_markov, threshold, comparator)
  return(icer_nmb)
}
######################################################################
#' Function to estimate the expected value of partial perfect information
...
#' @param param_file all parameters required to run the model,provided with name of
#' parameter, distribution and parameters that define the probability distribution
#' @param colnames_paramdistr coumn names where parameter distribution and values are defined
#' @param list_markov list of markov models to estimate the NMB
#' @param threshold threshold values of WTP
#' @param outer_iterations number of iterations for outer loop
#' @param inner_iterations number of iterations for inner loop
#' @param comparator optinal parameter if need ICER as the output
#' @return result result after evppi calculation, including three forms of evppi
...
estimate_evppi <- function(param_interest, param_file,colnames_paramdistr,
  ...
  all_mean_thetac_nmb_thetai = matrix(0, nrow = outer_iterations, ncol = cols_needed)
  all_mean_thetac_maxT_nmbs_thetai = matrix(0, nrow = outer_iterations, ncol = 1)
  all_maxT_mean_thetac_maxT_nmbs_thetai = matrix(0, nrow = outer_iterations, ncol = 1)
  ...
  for (outer_loop in 1:outer_iterations) {
    ...
    if (packDAMipd::check_null_na(value_param_interest) == 0 ) {
      if (!is.numeric(value_param_interest)) {
        stop("Value of parameter of interest should be numeric")
      }
    }else{
      stop("Value of parameter of interest can not be null or NA")
    }
    ...
    for (inner_loop in 1:inner_iterations) {
      ...
      check = colnames(this_result) %in% c("Strategy","Cost","Effect","NMB",
                                   "Inc_Cost","Inc_Effect","ICER")
      if (sum(check) != length(c("Strategy","Cost","Effect","NMB",
                                "Inc_Cost","Inc_Effect","ICER"))) {
        stop("Error - colnames from calculate_icer_nmb may not be right")
      }

      if (sum(is.na(suppressWarnings(as.numeric(this_result$NMB)))) != 0) {
        stop("NMB calculated should be numeric")
      }
      ...
      # store
      all_max_nmb_iteration[length(all_max_nmb_iteration) + 1] = maxT_nmbs_thetai
    }
    # step 6
    ...
    #store them
    all_mean_thetac_nmb_thetai[outer_loop, 1:cols_needed] = mean_thetac_nmb_thetai
    all_mean_thetac_maxT_nmbs_thetai[outer_loop] = mean_thetac_maxT_nmbs_thetai
    all_maxT_mean_thetac_maxT_nmbs_thetai[outer_loop] = maxT_mean_thetac_maxT_nmbs_thetai
  }
  #step 10
  mean_thetai_mean_thetac_nmb_thetai = colSums(all_mean_thetac_nmb_thetai)/outer_iterations
  mean_thetai_mean_thetac_maxT_nmb_thetai = mean(all_mean_thetac_maxT_nmbs_thetai)
  mean_thetai_maxT_mean_thetac_nmbs_thetai = mean(all_maxT_mean_thetac_maxT_nmbs_thetai)

  evppi = mean_thetai_mean_thetac_maxT_nmb_thetai - mean_thetai_maxT_mean_thetac_nmbs_thetai
  result = structure(list(
    max_mean_thetai_mean_thetac_nmb = max(mean_thetai_mean_thetac_nmb_thetai),
    ...
    mean_thetai_maxT_mean_thetac_nmbs = mean_thetai_maxT_mean_thetac_nmbs_thetai,
...
estimate_evpi <- function(param_file,colnames_paramdistr,
                           list_markov,threshold, iterations, comparator = NULL) {
...
find_nmb_allparams <- function(param_file,colnames_paramdistr,
  ...
  for (i in 1:nrow(list_markov)) {
    ...
    if (sum(check) != length(names_params_model))
      stop("Error - Parameters passed and returned to the model should be simmiar")
    params_passed = list_markov[i,]$list_param_values
    current_param_list <- get_all_parameter_list(names_params_needed,names_params_model,
                                             params_passed,param_file,colnames_paramdistr)
...
#' @source https://www.sciencedirect.com/science/article/pii/S1098301510605888
#' @export
estimate_evpi_evppi_single_param = function(param_interest, param_file,colnames_paramdistr,
                      list_markov,threshold, outer_iterations, inner_iterations, comparator = NULL){
  ...
  results_evpi = estimate_evpi(param_file,colnames_paramdistr,
                                list_markov,threshold,outer_iterations, comparator)
  results_evppi = estimate_evppi(param_interest, param_file,colnames_paramdistr,
                 list_markov,threshold, outer_iterations, inner_iterations,comparator)
...
#' @source https://www.sciencedirect.com/science/article/pii/S1098301510605888
#' @export
estimate_evpi_evppi_diff_threshold = function(parameter_of_interest, param_file,
                                colnames_paramdistr,
                                list_markov,threshold_values, outer_iterations = NULL,
                                inner_iterations= NULL, comparator = NULL){
  ...
  all_evppis = as.data.frame(0, nrow = length(threshold_values), ncol = num_trt_strategies + 3)
  ...
  for (jj in 1:number_threshold) {
    ...
    res_a_param = estimate_evpi_evppi_single_param(parameter_of_interest, param_file,
                              colnames_paramdistr,list_markov,this_threshold,
                              outer_iterations, inner_iterations, comparator)
    ...
    for (i in 1:num_trt_strategies) {
      ...
      names_strategy <- append(names_strategy,list_markov[i,]$strategy$name_strategy)
    }
    names_strategy <- unlist(names_strategy)
    all_evppis[jj, num_trt_strategies + 2] <- as.numeric(res_a_param$evppi[3])
    all_evppis[jj, num_trt_strategies + 3] <- as.numeric(res_a_param$evppi[4])
  ...
  colnames(all_evpis) <- c("Mean max NMB", "Max Mean NMB",
                           "Treatment with Max Mean NMB", "EVPI","Thresholds")
  temp_col_names = list()
  for (i in 1:num_trt_strategies) {
    temp_col_names <- append(temp_col_names, paste("Mean Thetai Mean Thetac NMB ",
                                                   names_strategy[i]))
  }
...
plot_evpi_threshold <- function(result_evpi_evppi){
  ...
  name_file_plot <- paste0("EVPI for threshold values for ", result_evpi_evppi$Parameter, ".pdf", sep = "")
  ...
  name_file_plot <- paste0("EVPPI for threshold values for ", result_evpi_evppi$Parameter, ".pdf", sep = "")
  grDevices::pdf(name_file_plot)
